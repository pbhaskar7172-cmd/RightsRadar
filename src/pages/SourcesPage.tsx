  ...
  return (
    <div className="space-y-8 animate-fadeIn text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="p-3.5 bg-amber-950/40 rounded-2xl border border-amber-500/30 text-xs text-amber-300 max-w-sm flex items-start gap-2 backdrop-blur-xl shadow-glow-amber">
      ...
      <div className="bg-slate-900/90 p-4 sm:p-5 rounded-3xl border border-slate-800 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-4 backdrop-blur-xl">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-3.5 py-2.5 text-xs sm:text-sm bg-slate-950 text-white placeholder:text-slate-500 rounded-xl border border-slate-800 focus:border-civic-500 outline-none transition-all"
          />
        </div>
      </div>
      ...
      {filteredSources.length > 0 ? (
        ...
        <div className="p-12 text-center bg-slate-900 rounded-3xl border border-dashed border-slate-800">
          ...
          <p className="text-xs text-slate-400 mt-1 mb-4">Try clearing your search query or selecting a different category.</p>
          <Button size="sm" onClick={() => { setSearchQuery(''); setSelectedType('all'); }} className="bg-slate-800 text-slate-200">Clear Filters</Button>
        </div>
      )}
        ...
        {selectedSource && (
          <div className="space-y-6">
            <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 text-xs text-slate-300 leading-relaxed">
              <span className="font-bold text-white block mb-1">Authoritative Summary:</span>
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-2">
                {selectedSource.keyTakeaways.map((t: string, idx: number) => (
                  <li key={idx} className="flex items-start gap-2.5 text-xs text-slate-200 bg-slate-950 p-3.5 rounded-xl border border-slate-800/80">
            ...
            <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
              <div className="text-xs text-slate-400">
                Issued by: <strong className="text-slate-200">{selectedSource.authority}</strong>
              </div>
              ...
              <a
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-civic-600 hover:bg-civic-500 text-white text-xs font-bold transition-all shadow-glow"
              >
